...
bool handiCap::EnableLightBox(bool enable)
{
    char command[handi_CMD];
    char response[handi_RES];
    ...
    if (enable)
        strncpy(command, ">L000", handi_CMD);
    else
        strncpy(command, ">D000", handi_CMD);

    if (!sendCommand(command, response))
        return false;

    char expectedResponse[handi_RES];
    if (enable)
        snprintf(expectedResponse, handi_RES, "*L000");
    else
        snprintf(expectedResponse, handi_RES, "*D000");
...
bool handiCap::SetLightBoxBrightness(uint16_t value)
{
    if (isSimulation())
    {
    ...
    snprintf(command, handi_CMD, ">B%03d", value);
